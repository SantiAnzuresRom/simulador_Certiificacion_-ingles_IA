...
export default function SpeakingModule() {
  ...
  const fetchTask = useCallback(async (uid: string) => {
    try {
      ...
      setTask({
        prompt: "Modo Offline",
        targetSentence: "Check your server connection.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const updateFirebaseProgress = async (finalScore: number) => {
    ...
    try {
      ...
      const listening = currentModuleData.listening || 0;
      const writing = currentModuleData.writing || 0;
      const levelAverage = Math.round(
        (reading + listening + writing + finalScore) / 4,
      );

      await updateDoc(progressRef, {
        [`${moduleKey}.speaking`]: finalScore,
  ...
  if (loading)
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#020617]">
        ...
        <p className="text-cyan-400 font-bold text-[11px] tracking-[0.4em] italic uppercase">
          Iniciando protocolo speaking...
        </p>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#020617] text-white flex flex-col font-sans selection:bg-cyan-500/30">
      ...
      <main className="max-w-4xl mx-auto w-full p-6 py-12 space-y-12 flex-grow">
        <section className="bg-slate-900/40 border border-white/5 p-12 rounded-[50px] text-center relative shadow-2xl overflow-hidden">
          ...
          <Sparkles
            className="absolute top-6 right-8 text-cyan-500/20"
            size={40}
          />

          <h2 className="text-[10px] font-bold text-cyan-500/60 italic tracking-[0.3em] mb-6 uppercase">
            Target_Sentence_v2.0
          </h2>
          <p className="text-4xl font-black italic tracking-tighter mb-10 leading-tight">
            "{task?.targetSentence}"
          </p>
          ...
          <div className="flex justify-center gap-6">
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 bg-white/5 rounded-full text-slate-400 hover:text-white hover:bg-white/10 transition-all text-[10px] font-bold italic tracking-widest border border-white/5 uppercase"
            >
        ...
        <section className="flex flex-col items-center gap-8">
          <div className="relative">
            ...
            <button
              ...
              className={`w-32 h-32 rounded-full flex items-center justify-center relative z-10 transition-all active:scale-90 ${
                isRecording
            ...
            >
              {isRecording ? (
                ...
                <Mic size={40} className="text-[#020617]" />
              )}
            </button>
          </div>
          ...
          <div className="w-full bg-slate-900/60 p-10 rounded-[40px] border border-white/5 text-center min-h-[140px] flex items-center justify-center relative">
            <div className="absolute top-4 left-1/2 -translate-x-1/2 flex gap-1.5">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className={`w-1.5 h-1.5 rounded-full ${isRecording ? "bg-red-500 animate-pulse" : "bg-slate-700"}`}
            ...
            {transcript ? (
              ...
              <p className="text-slate-500 italic text-[11px] font-bold tracking-[0.2em] uppercase">
          ...
          <button
            ...
            className="w-full py-8 bg-white text-[#020617] rounded-[30px] font-black italic uppercase text-[11px] tracking-[0.3em] disabled:opacity-10 hover:bg-cyan-400 transition-all flex items-center justify-center gap-4 shadow-2xl active:scale-95"
          >
        ...
        {showResult && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#020617]/98 backdrop-blur-2xl">
            <motion.div
              ...
              className="bg-[#0f172a] border border-cyan-500/20 p-16 rounded-[70px] text-center max-w-md w-full shadow-3xl"
            >
              <Award
                className="mx-auto text-cyan-400 mb-8 drop-shadow-[0_0_20px_rgba(34,211,238,0.5)]"
                size={100}
              />
              <div className="flex items-center justify-center gap-4 mb-4">
                <h2 className="text-9xl font-black italic text-white tracking-tighter">
                  {score}
                </h2>
                <span className="text-4xl font-black text-cyan-500 italic">
                  %
                </span>
              </div>
              <p className="text-cyan-500/50 text-[11px] font-bold italic tracking-[0.4em] mb-12 uppercase">
                Precisión vocal verificada
              </p>
              <div className="flex flex-col gap-4">
                <button
                  onClick={handleRetry}
                  className="w-full py-6 bg-white/5 border border-white/10 text-white rounded-[25px] font-bold italic uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 hover:bg-white/10 transition-all"
                >
                  <RotateCcw size={18} /> Reintentar fase
                </button>
                <button
                  onClick={() => router.push("/modulos")}
                  className="w-full py-7 bg-cyan-600 text-[#020617] rounded-[25px] font-black italic uppercase text-[11px] tracking-widest flex items-center justify-center gap-3 shadow-[0_20px_40px_rgba(8,145,178,0.3)] hover:bg-cyan-500 transition-all"
                >
                  <LayoutGrid size={18} /> Regresar a módulos
                </button>
              </div>
            </motion.div>
          </div>
