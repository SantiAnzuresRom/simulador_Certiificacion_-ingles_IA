...
// URL del Backend desde .env o local
const BACKEND_URL =
...
export default function RegisterPage() {
  ...
  const handleVerifyAndRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    if (otp.length !== 8) {
      // Ajustado a los 8 dígitos que genera el backend
    ...
    try {
      ...
      if (verifyRes.ok) {
        ...
        // 3. Guardar perfil extendido en Firestore vía Backend
        ...
        if (registerRes.ok) {
          ...
          alert(
            "Cuenta creada, pero hubo un error al guardar tu perfil. Contacta a soporte.",
          );
        }
      } else {
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      ...
      // Opcional: Registrar también en tu DB tras Google Sign In
      router.replace("/dashboard");
    } catch (error) {
      console.error(error);
    } finally {
    ...
    <div className="relative min-h-screen flex items-center justify-center bg-[#f8fafc] px-6 overflow-hidden">
      ...
      >
        ...
        <div className="hidden md:flex flex-col items-center justify-between bg-[#0f172a] p-12 text-white relative overflow-hidden">
          <div className="relative z-10 text-center w-full flex flex-col items-center justify-center">
            ...
            >
              <Image
                ...
                height={80}
                priority
                className="drop-shadow-[0_0_20px_rgba(135,206,235,0.4)] mb-4 object-contain"
              />
            </motion.div>
          ...
          <div className="relative z-10 w-full text-center">
            ...
            <div className="flex justify-center gap-8">
              ...
              >
                <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center transition-all group-hover:bg-[#1877F2] group-hover:shadow-[0_0_25px_rgba(24,119,242,0.5)] group-hover:-translate-y-2">
                  <Facebook
                    size={26}
              ...
              <a
                ...
                className="group flex flex-col items-center gap-3"
              >
                <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center transition-all group-hover:bg-white group-hover:shadow-[0_0_25px_rgba(255,255,255,0.2)] group-hover:-translate-y-2">
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
        ...
        <div className="px-10 py-10 flex flex-col justify-center bg-white overflow-y-auto max-h-[90vh] md:max-h-none">
          <AnimatePresence mode="wait">
            {step === 1 ? (
              ...
              >
                <div className="mb-6">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[9px] font-black text-[#87CEEB] uppercase tracking-[0.3em]">
                      Registry_Protocol
                    </span>
                ...
                <form className="space-y-3" onSubmit={handleInitialSubmit}>
                  ...
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="w-full rounded-2xl bg-slate-900 py-4 text-[11px] font-black text-white uppercase tracking-[0.2em] transition-all mt-4 hover:bg-[#87CEEB] hover:text-slate-900 flex justify-center items-center shadow-xl active:scale-[0.98] disabled:opacity-70"
                  >
                    {isLoading ? (
                      <Loader2 className="animate-spin" size={18} />
                    ) : (
                      <span className="flex items-center gap-2">
                        Enviar Código <ArrowRight size={14} />
                      </span>
                    )}
                  </button>
                </form>
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-3 border border-slate-200 rounded-2xl hover:border-[#87CEEB] hover:bg-[#87CEEB]/5 transition-all text-[10px] font-black text-slate-900 uppercase w-full"
                >
                  <img src="/google.svg" alt="G" className="h-4" /> Google_ID
                  Registry
                </button>
              </motion.div>
            ) : (
              ...
              >
                ...
                <h2 className="text-2xl font-black text-slate-900 mb-2 uppercase italic tracking-tight">
                  Verifica tu correo
                </h2>
                <p className="text-[10px] text-slate-500 mb-8 font-bold">
                ...
                <form onSubmit={handleVerifyAndRegister} className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full rounded-2xl bg-[#0f172a] py-4 text-white font-black uppercase tracking-widest hover:bg-green-500 transition-all shadow-xl active:scale-95 disabled:opacity-50"
                    >
                      {isLoading ? (
          ...
          <Link
            href="/login"
            className="mt-6 flex items-center justify-center gap-2 text-center text-[10px] text-slate-400 hover:text-[#87CEEB] transition-colors font-black uppercase tracking-widest group"
          >
            <Image
              src="/logo2.png"
              alt="L2"
